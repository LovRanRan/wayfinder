...
servers for code facts, verifies high-risk claims with tests, and writes final
...
`verifier` checks high-risk claims. It uses `mcp-test-runner` to run minimal
pytest or Jest targets when a claim is concrete enough to test. It does not try
to verify every sentence. Source locations, signatures, and direct references
already come from deterministic tools. Runtime behavior, numeric claims, state
mutations, and testable error paths are the claims that need executable
evidence.
...
`verifier` 负责高风险 claim。它通过 `mcp-test-runner` 跑最小 pytest/Jest target。它不会验证每一句话，因为 source location、signature、reference 这些已经来自 deterministic tool。真正需要 executable evidence 的，是 runtime behavior、数字、状态变化、错误路径、测试结果这类 claim。
