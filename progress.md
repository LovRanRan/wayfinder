...
| **Last Activity**      | 2026-07-02 ·**Enterprise hardening sprint (Phases 1–3 + UI tokens)**: 2 real bugs fixed with repro tests (claim double-count dedupe guard; local-path allowlist `WAYFINDER_LOCAL_REPO_ROOTS`), 4 agent-reported HIGHs verified as false positives and left untouched; CORS + `/ready` probe + structured JSON logs + request-id + stdlib rate limit; **PostgresRunStore** (psycopg3 adapter reusing all SQLite method bodies, live-DB verified); dashboard light-first SaaS design tokens. Gate: 336/9, ruff/mypy clean, dashboard build green. Earlier 2026-06-22: 3 grounding fixes live on CloakBrowser.  |
...
| **Next Action**        | **Phase 4 UI 剩余**(Cowork 继续,Haichuan 已授权):AppShell 侧边栏+面包屑 → 多页路由(/threads /history /metrics /settings /runs/[jobId]) → Toast/骨架屏/空状态 → 拆 966 行 `RepoConversationWorkspace` + `usePolling`。之后可选:Railway 切 `WAYFINDER_RUN_STORE=postgres`。仍欠(Haichuan-owned):notes 021–025 + Commit 23 reverse-explanation pass、verifier 去重守卫 + postgres 适配器 reverse-explanation、3-min recursive demo video。 |
...
- **Phase 1 — Bug 修复(2 真 / 4 误报,逐条人工核实)**:
  - **真 #1 claim 双重计数**(`graph/verifier.py`):同一 claim text 可同时出现在 AST-verified 与 test-unverified 两个桶(`initial_verified_claims=ast_verified_claims` 与 test 结果桶无去重),verification summary 会多算/错算。修复:`_verification_state` 汇聚点加 `_dedupe_claims_by_precedence` 守卫,优先级 `contradicted > verified > unverified`,同 text 只落一个桶;附复现测试(AST-verified + failing test → 只剩 contradicted,总数 1)。
  - **真 #2 本地路径任意读**(`ingestion/resolver.py` + `api/main.py`):`repo_url=../../../etc` 之类的本地路径无根约束即可被 resolve 并送进 scanner(GitHub 路径有 allowlist,本地路径反而没有)。修复:resolver 加 `allowed_roots` 参数(None=dev 放行 / 空=全拒 / 列表=前缀匹配),API 层新增 `WAYFINDER_LOCAL_REPO_ROOTS`(os.pathsep 分隔),auth-required 模式默认拒绝所有本地路径 → 403。+5 测试。
  - **误报(核实后未改,防止"修"正确代码)**:secret_box XOR 实为 encrypt-then-MAC(SHA256-HMAC keystream + `compare_digest`,解密前验 tag),非可篡改;run_store 会话过期/settings 读取均在 `self._lock` 内,无竞态;sandbox `tests/..` 会被 `".." in parts` 拦截,`::` 后是 pytest node-id 非路径;dev-user fallback 是 `REQUIRE_AUTH=0` 的按设计行为。
- **Phase 2 — 企业加固(全部无新依赖、默认 no-op、env 开关)**:新 `api/logging_config.py`(JSON 结构化日志 + contextvars request_id)与 `api/middleware.py`(request-id 中间件 + 访问日志 + stdlib 固定窗口限流 `WAYFINDER_RATE_LIMIT_PER_MINUTE` → 429);CORS `WAYFINDER_CORS_ALLOW_ORIGINS`;`GET /ready` readiness 探针(区别于 `/health` liveness,检查 run store `ping()`,SQLite 卷丢失时可被编排器摘流量);响应带 `x-request-id`。+5 测试。
- **Phase 3 — Postgres run store(psycopg3,Haichuan 选定)**:新 `api/postgres_store.py`。设计要点:**不重写 34 个方法**,`PostgresRunStore` 继承 `SQLiteRunStore` 全部方法体,由 `_PgConnection` 适配器透明翻译——`translate_sql`(`?`→`%s`、`INSERT OR REPLACE`→`ON CONFLICT (pk) DO UPDATE`)、`UniqueViolation`→`sqlite3.IntegrityError` 映射、`with conn` 语义修正(psycopg3 的 `__exit__` 会关连接,适配器只 commit/rollback——这是唯一真踩的坑,smoke 首跑即暴露)。schema DDL 抽成共享常量 `SCHEMA_DDL`(SQLite/PG 方言交集),单一 SQL 源不漂移。工厂:`WAYFINDER_RUN_STORE=postgres` + `WAYFINDER_DATABASE_URL`;pyproject 加 `postgres` extra(psycopg[binary]≥3.2)。**在本机真实 Postgres 上端到端验证**:用户/重复注册/认证/run 生命周期/thread/message/memory/active-context 全通;留下 env-gated 集成测试(`WAYFINDER_RUN_POSTGRES_INTEGRATION=1` + `WAYFINDER_TEST_DATABASE_URL`,与 Project 5 MCP 集成测试同款闸门)+ 3 个免 DB 的 translate_sql 单测。
- **Phase 4 — UI 网页化(仅基础层,未完)**:`dashboard/app/globals.css` + `tailwind.config.ts` 重写为 light-first SaaS token(indigo 主色/slate 中性/白卡片,shadow-sm/md/lg 层级,display/title/heading 字号档,focus-visible ring,`.dark` 整套暗色可切换)。**剩余(未做)**:AppShell 持久侧边栏+面包屑、多页路由拆分(/threads /history /metrics /settings /runs/[jobId])、Toast/骨架屏/空状态、拆 966 行 `RepoConversationWorkspace` + `usePolling` hook 去重。
- **验证方式**:`ruff check .` clean;`mypy src tests` clean(77 files);`uv run pytest -q` → **336 passed / 9 skipped**(从 324/8 起,新增 ~15 测试);dashboard `lint + typecheck + build` 全绿;Postgres 集成测试在临时库 `wayfinder_pgtest` 实测 4 passed 后 drop。`.env.example`/README 已同步新 env(LOCAL_REPO_ROOTS/CORS/RATE_LIMIT/LOG_FORMAT/DATABASE_URL/`/ready`)。
- **谁写的(诚实记录)**:Cowork 全程实现,Haichuan 明确授权并做了三次方向决策(优先级"先 bug 再加固最后 UI"、UI 走"现代 SaaS 亮色"、数据层"加 Postgres 选项用 psycopg3")。探索 agent 的 6 个 HIGH 发现里 4 个被 Cowork 人工核实为误报——**只修了有复现测试的 2 个真 bug**。本次 commit+push 由 Haichuan 明确指示 Cowork 执行。按 rule 16,verifier 去重守卫与 postgres 适配器的 reverse-explanation 归 Haichuan 所有权债。
- **下一步**:Phase 4 剩余(AppShell → 多页路由 → Toast/骨架屏 → 拆大组件);之后按需把 Railway API 服务切 `WAYFINDER_RUN_STORE=postgres`(需先建 Railway Postgres 插件)。
...
- **状态**:**CLOSED — won't-do(2026-06-22, David 决定作废)**。从未 commit,代码里不存在。作废理由:对 factual 无增益,citation 提升来自 harness resolver 而非此 prompt;且 2026-06-22 三项 grounding 修复上线后,线上答案本就符号密、backtick 规范(`resolve_proxy_geo @ geoip.py:54`),该效果已自然达成,不需此 prompt。实验记录保留备查,详见 agent-eval-harness `LEARNINGS.md`「citation metric fix」。
...
- **做了什么**:Product consolidation toward a single ambient surface (the original Commit 22 intent of folding Run/Answer into Threads). Retired the **Run** tab: removed it from `WorkspaceTab` union + `WorkspaceTabs` list (6→5 tabs, `Play` icon dropped), deleted the `activeTab === "run"` block and `RunLauncher`/`RunBriefingPanel` imports in `agent-workbench.tsx`, cleaned the `?tab=run` URL-sync branch and `workspaceTabFromParam`. Threads now owns all input (attach repo + ask + report/evidence-mode grounded runs); Answer remains the structured deep-view reachable by selecting a run.
- **同测试时修的 UI**:contradiction-card "error" mislabel fix; no-active-repo send now blocked with "open a repo first" (was silently wiping input); ChatGPT-style viewport-locked workspace (pinned composer, internal scroll, page no longer grows) across `page.tsx` + `agent-workbench.tsx` + `repo-conversation-workspace.tsx`.
- **边界 / deferred**:Did NOT delete Answer or fully fold its rich provenance/evidence view into the in-thread grounded-report attachment — that needs the in-thread attachment to match the Answer console's richness first (deferred follow-up). `run-launcher.tsx` / `run-briefing-panel.tsx` are now orphaned files (safe; optional `git rm`).
- **验证方式**:Edits are className/JSX/type-union changes; needs `cd dashboard && npm run lint && npm run typecheck && npm run build` by Haichuan (I can't run the npm gate). Live manual smoke B–G passed before this change.
- **下一步**:Run the frontend gate, commit/push, redeploy, confirm Run tab is gone and Threads/Answer/History/Metrics/Settings still work. Then optionally fold Answer into Threads (Commit 24 follow-up) + the still-owed Commit 23 reverse-explanation pass.
...
- **Codex 帮了哪里**:Codex wrapped `supervisor`, `architect_mapper`, `entry_explainer`, and `final_writer` nodes, added a regression test with a deliberately slow architecture scanner, documented the Railway variable, and fixed dashboard polling so one transient status fetch failure no longer leaves the UI stuck in `running`. `verifier` stays unwrapped because LangGraph `interrupt()` requires runnable context and verifier execution already has sandbox/test timeouts.
- **验证方式**:`uv run pytest tests/test_graph_contract.py tests/test_verifier.py -q`(24 passed);`uv run ruff check .`;`uv run mypy src tests`;`uv run pytest -q`(240 passed,8 skipped);dashboard `npm run lint`;dashboard `npm run typecheck`;`git diff --check`. Local `next build` was not completed because the local Next build process hung after cache cleanup and was terminated.
...
- **做了什么**:Added Commit 20 to the Project 6 roadmap as the explicit path from `WAYFINDER_VERIFIER_RUNNER=placeholder` to a real `sandboxed_mcp` worker.
- **自己设计了什么**:Separated Commit 19 public launch closeout from Commit 20 execution safety. Commit 20 owns the separate worker service, request schema, command-denial policy, API adapter, dashboard approval, denied-operation evidence, and deploy topology.
- **验证方式**:Tracker-only update;run `git diff --check` before commit.
- **问题记录**:Commit 20 is now planned but not implemented. Railway should remain `WAYFINDER_VERIFIER_RUNNER=placeholder` until the worker and health gate exist.
- **下一步**:Finish Commit 19 video/evidence closeout, then start Commit 20 by writing `docs/design_notes/016_sandboxed_test_runner_worker.md`.
...
- **Hotfix after public smoke**:A `langchain-ai/langchain` architecture run exposed a `mcp-repo-mapper scan_repo` UTF-8 decode failure. Added architecture scanner degradation so tool exceptions become `architecture_scan_tool_error` limitations and the graph can still complete instead of returning a failed API job. A second public smoke showed reflection-cap noise because the LLM named the core UTF-8 limitation without repeating the full raw error string;reflection matching now accepts meaningful message fragments.
- **Hotfix verification**:`uv run pytest -q`(201 passed,8 skipped);`uv run ruff check .`;`uv run mypy src tests`.
...
- **自己设计了什么**:Kept deploy evidence honest in two phases:first CLI snapshot deploy from commit `c1c09c4`, then Haichuan completed the Railway UI GitHub connection for both services.
- **更新**:API deployment `7d4db40e-fa5b-4627-b3fe-ef11839114f6` and dashboard deployment `ca1b1332-f7be-4014-8dc5-bc455e3cfc3f` both succeeded from the updated GitHub-backed Railway service configuration.
...
- **问题记录**:Dashboard CLI deploy must use `railway up --service wayfinder-dashboard --path-as-root dashboard`;GitHub-backed deploy must instead keep Railway service root directory set to `/dashboard`. API service must keep `RAILWAY_DOCKERFILE_PATH=Dockerfile.api`.
- **下一步**:Record the 3-min demo video.
